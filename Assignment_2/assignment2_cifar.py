...
## load 1st data
X1_train = pickle.load(open("./cleaned_data/cifar_x_train_1", "rb"))
X1_test = pickle.load(open("./cleaned_data/cifar_x_test_1", "rb"))



## load 2nd data
X2_train = pickle.load(open("./cleaned_data/cifar_x_train_2", "rb"))
X2_test = pickle.load(open("./cleaned_data/cifar_x_test_2", "rb"))


## load 3rd data
X3_train = pickle.load(open("./cleaned_data/cifar_x_train_3", "rb"))
X3_test = pickle.load(open("./cleaned_data/cifar_x_test_3", "rb"))


## load 4th data
X4_train = pickle.load(open("./cleaned_data/cifar_x_train_4", "rb"))
X4_test = pickle.load(open("./cleaned_data/cifar_x_test_4", "rb"))
...
Y_train = pickle.load(open("./cleaned_data/cifar_y_train", "rb"))
Y_test = pickle.load(open("./cleaned_data/cifar_y_test", "rb"))
...
for i in range(0,n):
    ...
    cbks_autoencoder = [callbacks.ModelCheckpoint(filepath='./cifar_models/autoencoder_model'+str(i)+'.h5')]
    ...
    # save the classifier
    classifier.save('./cifar_models/newmodel_model'+str(i)+'.h5')
...
for k in range(0,epochs):   
    ...
    for i in range(0,int(len(final_X_train)/size_batch)):
        ...
        for j in range(0,n):
            autoencoder = load_model('./cifar_models/autoencoder_model'+str(j)+'.h5')
        ...
        if threshold > np.min(score_for_autoencoder):
            ...
            threshold = 0.15
            autoencoder = load_model('./cifar_models/autoencoder_model'+str(min_error_index)+'.h5')  
            classifier = load_model('./cifar_models/newmodel_model'+str(min_error_index)+'.h5')
        else:
            print('A new autoencoder and classifier is created')
            add_autoencoder = True
            autoencoder, classifier = create_autoencoder_classifer(input_shape)
            autoencoder.save('./cifar_models/autoencoder_model'+str(n)+'.h5')
            classifier.save('./cifar_models/newmodel_model'+str(n)+'.h5')
            # add extra classifer
            n = n+1
            threshold = 0.7
        ...
        if add_autoencoder == True:
            ...
            autoencoder.save('./cifar_models/autoencoder_model'+str(min_error_index)+'.h5')
            classifier.save('./cifar_models/newmodel_model'+str(min_error_index)+'.h5')
